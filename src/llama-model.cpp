...
bool llama_model::load_tensors(llama_model_loader & ml) {
    ...
    // create tensors for the weights
    {
        ...
        auto create_tensor = [&](const LLM_TN_IMPL & tn, const std::initializer_list<int64_t> & ne, int flags) -> ggml_tensor * {
            ...
            // if (!t_meta && flags & TENSOR_PREFETCH) {
            //     t_meta = ggml_new_tensor();
            // }
            ...
            // sanity checks
            if (info.layer == LLM_TENSOR_LAYER_INPUT || info.layer == LLM_TENSOR_LAYER_OUTPUT || info.layer == LLM_TENSOR_LAYER_PREFETCH) {
                if (tn.bid != -1) {
            ...
            switch (info.layer) {
                ...
                case LLM_TENSOR_LAYER_REPEATING:
                    ...
                    break;
                case LLM_TENSOR_LAYER_PREFETCH:
                    buft_list = pimpl->dev_layer.at(0).buft_list;
                    break;
                default:
            ...
            case LLM_ARCH_QWEN3MOE:
                {
                    ...
                    {
                        const int64_t n_ff_exp = hparams.n_ff_exp ? hparams.n_ff_exp : n_ff / n_expert_used;
                        const int64_t n_prefetch_experts = GGML_MAX_PREFETCH_NUM;
            ...
            case LLM_ARCH_DEEPSEEK2:
                {
                    ...
                    {
                        const int64_t n_ff_exp = hparams.n_ff_exp ? hparams.n_ff_exp : n_ff / n_expert_used;
                        const int64_t n_prefetch_experts = 512;
                        prefetch_gate_wgt = create_tensor(tn(LLM_TENSOR_PREFETCH_GATE_EXPS, "weight"), {n_embd, n_ff_exp, n_prefetch_experts}, TENSOR_NOT_REQUIRED);
                        prefetch_down_wgt = create_tensor(tn(LLM_TENSOR_PREFETCH_DOWN_EXPS, "weight"), {n_ff_exp, n_embd, n_prefetch_experts}, TENSOR_NOT_REQUIRED);
                        prefetch_up_wgt =   create_tensor(tn(LLM_TENSOR_PREFETCH_UP_EXPS,   "weight"), {n_embd, n_ff_exp, n_prefetch_experts}, TENSOR_NOT_REQUIRED);
                    }
        ...
        for (int il = 0; il < n_layer; ++il) {
            ...
            ggml_tensor * moe_out =
                build_moe_ffn(cur,
                        ...
                        model.layers[il].ffn_gate_exps,
                        model.layers[il].ffn_down_exps,
                        model.prefetch_up_wgt,
                        model.prefetch_gate_wgt,
                        model.prefetch_down_wgt,
                        nullptr,
                        n_expert, n_expert_used,
                        LLM_FFN_SILU, true,
                        false, 0.0,
