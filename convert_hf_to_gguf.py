        ...
        if name.find("experts") != -1:
            ...
            if len(self._experts[bid]) >= n_experts * 3:
                ...
                for w_name in ["down_proj", "gate_proj", "up_proj"]:
                    ...
                    if bid == 0: # only the first layer, i.e. only once
                        merged_name = f"model.prefetch.{w_name}.weight"
                        new_name = self.map_tensor_name(merged_name)
                        data_stack = torch.cat([data_torch] * 8, dim = 0) # copy the data_torch three times and stack it together.
                        tensors.append((new_name, data_stack))
        ...
        if name.find("mlp.experts") != -1:
            ...
            if len(self._experts[bid]) >= n_experts * 3:
                ...
                for w_name in ["down_proj", "gate_proj", "up_proj"]:
                    ...
                    if bid == 1: # only the first layer, i.e. only once
                        merged_name = f"model.prefetch.{w_name}.weight"
                        new_name = self.map_tensor_name(merged_name)
                        data_stack = torch.cat([data_torch] * 8, dim = 0) # copy the data_torch three times and stack it together.
                        tensors.append((new_name, data_stack))
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--outtype", type=str, choices=["f32", "f16", "bf16", "q8_0", "tq1_0", "tq2_0", "auto", "q4_0"], default="f16",
