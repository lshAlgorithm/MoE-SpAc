...
## File modified based on llama.cpp
```
 .gitignore                       |   1 +
 README.md                        |  68 ++++++++++++++++
...
```
I ensure there is no personal information about the authors of MoE-SpAc.
...
1. Add Prefetch flags in `LLM_ARCH_[YOUR MODEL]` in file `src/llama-arch.cpp`:
    ```cpp
    { LLM_TENSOR_PREFETCH_GATE_EXPS, "prefetch_gate_wgt" },
    { LLM_TENSOR_PREFETCH_DOWN_EXPS, "prefetch_up_wgt" },
    { LLM_TENSOR_PREFETCH_UP_EXPS,   "prefetch_down_wgt" },
    ```
1. Add `prefetch_wgt` in `src/llama-model.cpp`
    ```cpp
    {
...
3. Run the demo
    ...
    ./build/bin/llama-speculative \
        ...
        -e -ot "\\.ffn_(up|down|gate)_exps"=CPU -n 512 -c 4096 -s 8 --draft 8 
    ```
4. Use the server as a client. Reference: https://rustman.dev/blog/chat-with-llama.cpp
    ...
    ./build/bin/llama-server \
        ...
        -ot "\\.ffn_(up|down|gate)_exps"=CPU -fa auto --port 8033 -c 4096 \
        --draft-max 8 -s 42
    ```
# llama.cpp
