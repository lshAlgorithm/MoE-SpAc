    ...
    if (ith == 0) {
        ...
        for (int64_t iid1 = 0; iid1 < ids->ne[1]; ++iid1) {
            for (int id = 0; id < n_ids; ++id) {
                ...
                // SpAc: refine the value of matrix_row_counts, and never map it, 
                //       then it will get `continued` in the for loop after barrier,
                //       without changing the shape of ids, and leave the final res to 0.
                bool not_used = false;
                if (i02 < 0) {
                    i02 += GGML_MAX_EXPERTS_NUM;
                } else {
                    not_used = true;
                }

                assert(i02 >= 0 && i02 < GGML_MAX_PREFETCH_NUM);
                
                if (!not_used) {
                    assert(i02 >= 0 && i02 < ne02);
                    MMID_MATRIX_ROW(i02, matrix_row_counts[i02]) = (struct mmid_row_mapping) {id, iid1}; // {id, token}
...
static void ggml_compute_forward(struct ggml_compute_params * params, struct ggml_tensor * tensor) {
    ...
    switch (tensor->op) {
        ...
        case GGML_OP_AGGRE:
            {
                ggml_compute_forward_aggre(params, tensor);
            } break;
        case GGML_OP_DIV:
            {
