...
static inline float op_aggre(float a, float b) {
    GGML_ASSERT(fabs(a) < 1e-3f || fabs(b) < 1e-3f);
    return a + b;
}
...
void ggml_compute_forward_aggre(const ggml_compute_params * params, ggml_tensor * dst) {
    binary_op<op_aggre>(params, dst);
}
