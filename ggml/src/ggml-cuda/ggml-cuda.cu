...
static __global__ void aggregate_f32(const float * x, const float * y, float * dst, const int ne0) {
    int64_t offset = threadIdx.x + blockIdx.x * blockDim.x;
    if (offset >= ne0) {
        return;
    }

    offset +=   blockIdx.y * ne0 + 
                blockIdx.z * ne0 * gridDim.y;
    
    if (!(fabs(x[offset]) < 1e-3f || fabs(y[offset]) < 1e-3f)) {
        printf("[%f, %f]\t", x[offset], y[offset]);
    }
    assert(fabs(x[offset]) < 1e-3f || fabs(y[offset]) < 1e-3f);
    dst[offset] = x[offset] + y[offset];
}

static void ggml_cuda_aggre(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    GGML_ASSERT(src0->ne[1] == dst->ne[1]);
    ...
    GGML_ASSERT(ne3 == 1 && "aggregate_f32 only supports 3D tensors");
    GGML_ASSERT(ggml_is_contiguous(dst));
    dim3 gridDim(num_blocks, ne1, ne2);

    aggregate_f32<<<gridDim, CUDA_CONCAT_BLOCK_SIZE, 0, stream>>>(src0_d, src1_d, dst_d, ne0);
}

static void ggml_cuda_mul_mat_id(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    // ggml_set_zero(dst); // SpAc: set the result to 0
    ...
    // Zeroes out the memory for float16 initialization
    CUDA_CHECK(cudaMemsetAsync(dst_sorted.ptr, 0, dst_sorted.actual_size, stream));
    CUDA_CHECK(cudaMemsetAsync((char *)dst->data, 0, ggml_nbytes(dst), stream));
...

    {
        ...
        for (int64_t i02 = 0; i02 < ne02; ++i02) { // expert matrices
            for (int64_t i12 = 0; i12 < ne12; ++i12) { // tokens
                for (int64_t iex = 0; iex < n_expert_used; ++iex) {
                    ...
                    // SpAc: logic opposite to CPU backend
                    bool not_used = false;
                    if (expert_to_use < 0) {
                        expert_to_use += GGML_MAX_EXPERTS_NUM;
                        not_used = true;
                    }

                    assert(expert_to_use >= 0 && expert_to_use < ne02);
                    if (expert_to_use == i02) {
                        // decent map: `from_sorted` maps expert id to `to_sorted` index; 
                        //              while the latter map back to `from_sorted` index.
                        if (!not_used) {
                            ids_from_sorted_host[i12*n_expert_used + iex] = ids_to_sorted_host.size();
                            ids_to_sorted_host.push_back(i12*ne11 + iex % ne11); // map back to one token (add up)
                            tokens_per_expert[i02]++;
                        } else {
                            // set a temporary map, then move it to the two-way map.
                            tmp.push_back({i12*n_expert_used + iex, i12*ne11 + iex % ne11});
                        }
                        // break;
                    }
                }
        ...
        for (auto c: tmp) {
            ids_from_sorted_host[c.first] = ids_to_sorted_host.size();
            ids_to_sorted_host.push_back(c.second);
        }
    }
    ...
    char * src1_data_cur = (char *) src1_sorted.ptr; // the activation value of tokens(src1)
    char *  dst_data_cur = (char *)  dst_sorted.ptr; // the result of one slice
